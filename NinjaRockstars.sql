...
SELECT "Patient_ID", "FirstName","LastName" ,"DateOfBirth" FROM "Patients" order by "DateOfBirth" DESC

--2. Display the firstname and lastname of patients who speaks English language.--
SELECT "FirstName", "LastName","Language" 
FROM "Patients" P JOIN  "Language" L 
...
join public."PrimaryDiagnosis" as pd 
...
SELECT P."FirstName" || ' ' || P."LastName" AS PatientName ,"ReasonForVisit"
FROM "Patients" P JOIN "EDVisits" ED ON P."Patient_ID" = ED."Patient_ID"
JOIN "ReasonForVisit" RV ON ED."Rsv_ID" = RV."Rsv_ID"
...
FROM "AmbulatoryVisits" AV JOIN "VisitTypes" VT ON AV."AMVT_ID" = VT."AMVT_ID"
...
select Distinct("Patient_ID"),"Race","Language" 
...
on p."Gender_ID"= g."Gender_ID" where g."Gender"='Male';
...
--23. Find the category of illness(Stomach Ache or Migraine) that has maximum number of patients--

select COUNT(edv."Patient_ID") AS P_Count,rfv."ReasonForVisit" from public."EDVisits" as edv
join public."ReasonForVisit" as rfv on edv."Rsv_ID"=rfv."Rsv_ID"
where "ReasonForVisit"='Stomach Ache' OR "ReasonForVisit"='Migraine'
GROUP BY rfv."ReasonForVisit" ORDER BY P_Count DESC LIMIT 1
...
INSERT INTO "ReAdmissionRegistry" VALUES (1000,1000,'2018-05-26','2018-05-26 20:40:34','DID01','SID01','PD001','12','0.23',1,3,1)
--creating temporary table--

CREATE TABLE temp_readm(
...
BEGIN	
	INSERT INTO temp_readm("Entry_Date")
	VALUES(CURRENT_TIMESTAMP);
	RETURN NEW;
END;
$NEW$ LANGUAGE plpgsql;

--creating trigger named Trigger_readm--
CREATE TRIGGER Trigger_readm
AFTER DELETE ON "ReAdmissionRegistry"
FOR EACH ROW
EXECUTE PROCEDURE Del();

--Deleting a Record From ReAdmissionRegistry to execute Trigger
DELETE FROM "ReAdmissionRegistry" WHERE "Patient_ID"=1000

--Temp Table got updated with the time the record got deleted
SELECT * FROM temp_readm
...
SELECT P."FirstName" || ' ' || P."LastName" AS PatientName ,"VisitStatus"
FROM "Patients" P
...
SELECT DISTINCT P."Patient_ID","ReasonForVisit"
FROM "Patients" P
...
SELECT P."FirstName",P."LastName","Gender","Language","Race"
...
select "FirstName" || ' ' || "LastName" as "DischargedPatientName" ,"DischargeDisposition"
...
join public."Discharges" as d on d."Patient_ID"=p."Patient_ID"
join public."DischargeDisposition" as dd on d."Discharge_ID"= dd."Discharge_ID" 
...
join public."ProviderSpeciality" as ps on p."PS_ID"=ps."PS_ID"
...
consider readmission/discharges and ambulatory visits table for inpatients and outpatients*/
...
GROUP BY mns,yrs	)
SELECT mns,yrs,typ,counts FROM
  cte $$,
    $$ values
	(   'inpatients'  ),
	  ('outpatients')
...
/* Q58: Write a query to Count number of unique patients EDDisposition wise.*/

select EDD."EDDisposition", count(distinct("Patient_ID")) as "UniqueCountOfPatients"
...
date_part('year',age("DateOfBirth")) as Patient_Age , "Gender" , "BloodPressureSystolic","BloodPressureDiastolic"
from public."Patients" as p
join public."Gender" as g
...
date_part('year',age("DateOfBirth")) between 40 and 60 ;


--62. Query to find the number of outpatients who have visited month wise(use month names)--

select
count(p."Patient_ID") AS OutPatients_Count,to_char(av."DateofVisit",'month') month_visit,date_part('year',av."DateofVisit") year_visit
...
SELECT SUM("ExpectedLOS") AS ExpectedLOS_Sum
FROM "ReAdmissionRegistry" 
WHERE "Service_ID" = 'SID01'
...
SELECT "Service_ID","ExpectedLOS",PERCENT_RANK() OVER ( PARTITION BY "Service_ID" ORDER BY "ExpectedLOS") AS Percent FROM "Discharges"
...
, Floor("Pulse") as "heartrate" from "AmbulatoryVisits"
offset 1 Limit 21
